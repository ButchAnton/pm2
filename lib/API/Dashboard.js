...
Dashboard.init = function(opts) {
  ...
  // False while the help line sits on the last row and the host-metrics
  // line is hidden. See setSpeedbarLayout.
  ...
  // Same host-metrics line as `pm2 ls`, drawn with blessed tags. Hidden
  // until getSystemData returns something, so it takes no row when
  // pm2:sysmonit is off.
...
/**
 * Refresh the host-metrics line under the help bar.
 * @method refreshSystemData
 * @param {Object} systemdata result of getSystemData; empty when sysmonit is off
 * @return this
 */
Dashboard.refreshSystemData = function(systemdata) {
  ...
  var line = Dashboard.buildSpeedbarLine(systemdata);
...
/**
 * Blessed-tag form of the `host metrics` row in lib/API/UX/pm2-ls.js.
 * Keep the two in sync if one changes.
 * @method buildSpeedbarLine
 * @param {Object} m axm_monitor-shaped system snapshot
 * @return {String} empty when there is nothing to show
 */
Dashboard.buildSpeedbarLine = function(m) {
  ...
  var cpuSeg = '{bold}cpu{/}: ' + colorizeMetric(cpu.value, 40, 70, '%');
  var temp = m['CPU Temperature'] ? m['CPU Temperature'].value : null;
  if (temp && temp != '-1')
    cpuSeg += ' ' + colorizeMetric(temp, 50, 70, 'º');
  segs.push(cpuSeg);
  ...
  if (ramUsage !== null && typeof ramUsage !== 'undefined')
    segs.push('{bold}ram usage{/}: ' + colorizeMetric(ramUsage, 70, 90, '%'));
  ...
  if (m['graphics:mem:total'] && Number(m['graphics:mem:total'].value) > 0) {
    ...
    var gpuSeg = '{bold}gpu{/}: ' + gpuUsed + '/' + gpuTotal + 'mb';
    var gpuTemp = m['graphics:temp'] ? m['graphics:temp'].value : null;
    if (gpuTemp && gpuTemp != '-1')
      gpuSeg += ' ' + colorizeMetric(gpuTemp, 50, 70, 'º');
    segs.push(gpuSeg);
  }
  ...
  interfaces.forEach(function(iface) {
    ...
    if (!(Number(rx) > 0 || Number(tx) > 0))
      return;
    var netSeg = '{bold}' + iface + '{/}: ⇓ ' + rate(rx, 10, 20) + ' ⇑ ' + rate(tx, 10, 20);
    ...
    if (rxErr + txErr > 0)
      netSeg += ' {bold}err{/} ' + colorizeMetric(rxErr + txErr, 1, 10, '/min');
    if (rxDrop + txDrop > 0)
      netSeg += ' {bold}drop{/} ' + colorizeMetric(rxDrop + txDrop, 1, 10, '/min');
    segs.push(netSeg);
  });
  ...
  if (m['Disk Reads']) {
    ...
    var diskSeg = '{bold}disk{/}: ⇓ ' + rate(read, 10, 20) + ' ⇑ ' + rate(write, 10, 20);
    ...
    disks.forEach(function(fs) {
      ...
      if (use > 60)
        diskSeg += ' {grey-fg}' + shortMount(fs) + '{/} ' + colorizeMetric(use, 80, 90, '%');
    });
    segs.push(diskSeg);
  }

  return '{bold}{cyan-fg}host metrics{/} | ' + segs.join(' | ');
}

module.exports = Dashboard;

// Loopback, tunnels, container bridges and virtual NICs are noise on one line.
// Same filter as miniMonitBar in lib/API/UX/pm2-ls.js.
var VIRTUAL_IFACE = /^(lo\d*|utun\d*|tun\d*|tap\d*|wg\d*|ppp\d*|docker\d*|br-[0-9a-f]+|veth[0-9a-f]*|virbr\d*|vmnet\d*|vboxnet\d*|bridge\d*|awdl\d*|llw\d*|anpi\d*|ap\d*|gif\d*|stf\d*)$/;
...
function setSpeedbarLayout(dashboard, showSpeedbar) {
  ...
  if (showSpeedbar) {
    dashboard.metadataBox.bottom = 2;
    dashboard.metricsBox.bottom = 2;
    dashboard.box4.bottom = 1;
    dashboard.speedbarBox.show();
  } else {
    dashboard.metadataBox.bottom = 1;
    dashboard.metricsBox.bottom = 1;
    dashboard.box4.bottom = 0;
    dashboard.speedbarBox.hide();
  }

  // blessed's incremental render does not erase cells left behind when a
  // box's edge moves. realloc() forces a full repaint on the next render.
  if (dashboard.screen && dashboard.screen.realloc)
...
function rate(mb, warn, alert) {
  mb = Number(mb) || 0;
  if (mb > 0 && mb < 1)
    return colorizeMetric(Math.round(mb * 1024), warn * 1024, alert * 1024, 'kb/s');
  return colorizeMetric(mb == 0 ? 0 : Number(mb.toFixed(1)), warn, alert, 'mb/s');
}

function shortMount(mount, max) {
  max = max || 24;
  if (mount.length <= max)
    return mount;
  var parts = mount.split('/').filter(Boolean);
